...
def run_training_pipeline(config: TrainingConfig) -> Dict[str, Any]:
    ...
    row_counts_by_step["00_ingest"] = {"row_count_out": int(len(inv_df)), "row_count_in": 0}
    ...
    row_counts_by_step["01_parse_projections"] = {"row_count_out": int(len(players_all)), "row_count_in": 0}
    ...
    row_counts_by_step["02_parse_contest_entries"] = {"row_count_out": int(len(parsed_entries)), "row_count_in": 0}
    ...
    enriched_tables = []
    unmatched_total = 0
    step03_inputs: List[Tuple[str, str, str]] = []
    ...
    for slate_id, grp in parsed_entries.groupby("slate_id", dropna=False):
        ...
        enriched, m = enrich_showdown_entries(
            grp,
            ply.rename(columns={"name_norm": "name_norm", "team": "team", "salary": "salary", "proj_points": "proj_points"}),
    ...
    step03 = writer.write_step(
        ...
        inputs=[],
        outputs=[],
        metrics={"unmatched_player_names": unmatched_total, "join_coverage": 0.0 if len(parsed_entries)==0 else float(len(enriched_entries)/len(parsed_entries))},
        persist_parquet=config.persist_step_outputs,
    )
    row_counts_by_step["03_join_and_enrich"] = {
    ...
    row_counts_by_step["04_optimal_and_gap"] = {
    ...
    for gpp_category, grp in enriched_with_gap.groupby("gpp_category", dropna=False):
        ...
        out_path.write_text(json.dumps(dist.model_dump(mode="json"), indent=2, sort_keys=True), encoding="utf-8")
        run_outputs.append(
            ManifestIO(path=str(out_path), checksum_sha256=sha256_file(str(out_path)), logical_name=f"target_distributions:{cat}")
        )
        dist_rows.append({"gpp_category": cat, "n_rows": int(len(grp)), "path": str(out_path)})

    dist_df = pd.DataFrame(dist_rows)
    step05 = writer.write_step(
        step_idx=5,
    ...
    row_counts_by_step["05_fit_target_distributions"] = {
    ...
    run_outputs.append(
        ManifestIO(
            path=str(entries_path), checksum_sha256=sha256_file(str(entries_path)), logical_name="entries_enriched.parquet"
        )
    )
    ...
    (writer.run_dir / "run_manifest.json").write_text(
        json.dumps(run_manifest.model_dump(mode="json"), indent=2, sort_keys=True), encoding="utf-8"
    )
